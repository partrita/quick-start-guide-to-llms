...
class ChatbotGPT:
    ...
    def __init__(self, namespace, index, engine, threshold=0.8, conversation_id=None):
    ...
    def load_conversation_from_db(self, conversation_id):
        if not self.supabase_client:
            ...
            return
        response = (
            self.supabase_client.table("conversation")
            .select("*")
    ...
    def start_new_conversation(self):
        ...
        if self.supabase_client:
            self.supabase_client.table("conversation").insert(
                dict(conversation_id=conversation_id, conversation=self.conversation)
            ).execute()
    ...
    def user_turn(self, message):
        ...
        best_result = get_results(
            self.index, message, "none", 3, self.namespace, self.engine
        )
        ...
        for result in best_result:
            ...
            if result["score"] >= self.threshold:
                ...
                print(
                    f"Adding context: {result['metadata']['text'][:50]}... with score {result['score']}"
                )
        ...
        chatgpt_response = (
            openai.ChatCompletion.create(
                model="gpt-4", temperature=0, messages=self.conversation
            )
            .choices[0]
        ...
        if self.supabase_client:
            ...
            ).eq("conversation_id", self.conversation_id).execute()
